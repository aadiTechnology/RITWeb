...
namespace BusinessLogic
{
    public class SalaryDetailsBL
    {
        ...
        {
            get { return moSalaryDetailsDC.PaidSalaryDifferences; }
        }

        public List<PaidSalaryDifference> SalaryDifferencePaidDetails
        {
            get { return moSalaryDetailsDC.SalaryDifferencePaidDetails; }
        }
        ...
        {
            return moSalaryDetailsDC.GetUserInOutDetails(aiSchoolId, aiAcademicYearId, aiUserId, aiUserStaffGroupId, dtFromDate, dtToDate, out lstEmployeeNoDetails);
        }
        ...
        {
            return moSalaryDetailsDC.GetAllSalaryChanges(aiUserId);
        }
        ...
        private bool ShowAdditionalColumns
        {
            get
            {
                if (miSchoolId == Constants.SchoolId.CKInstOfCulinaryArtAndHotelMgmt.ToInt())
                    return true;
                else
                    return false;
            }
        ...
        private bool ExcludeWeekends
        {
            get 
            {
                // Even if code of this property is similar now like property ShowAdditionalColumns, in future it can be different.
                if (miSchoolId == Constants.SchoolId.CKInstOfCulinaryArtAndHotelMgmt.ToInt())
                    return true;
                else
                    return false;
        ...
        private bool ApplyStandardPTFormula
        {
            get
            {
                if (miSchoolId == Constants.SchoolId.CKInstOfCulinaryArtAndHotelMgmt.ToInt())
        ...
        public DataSet GetSalaryDetailsDataset(int aiMonthId, int aiYear, int aiStaffGroupId, string asFilter, int aiPageIndex, int aiPageSize, bool abIsPageInit, bool abReLoad)
        {
            ...
            miSundaysCount = GetWeekdayCount(aiYear, aiMonthId, DayOfWeek.Sunday);
            iTotalDaysOfMonth = iTotalDaysOfMonth - miSundaysCount;
            ...
            if (!BasicDetails.IsStaticData)
            {
                ...
                foreach (UsersBasicDetails userDetails in UserDetails)
                {
                    ...
                    iDaysOfMonth = iDaysOfMonth - miSundaysCount;
                    ...
                    if (iUsersEarningsDeductionsCount > 0)
                        SetUsersEarningsDeductions(iDaysOfMonth, ref iEarningsSum, ref iDeductionSum, iRowIndex, dcTotalDays, UsersEarningsDeductions, userDetails.UserId);
                    else
                    ...
                    foreach (var EDFormula in EarningsDeductionsFormulae)
                    {
                        ...
                        DesignFormula(iDaysOfMonth, dcTotalDays, ref iEDFormulaValue, UsersEDForFormula, dictEDValue, ref sFormula, ref sTempFormula, ref iTempEDValue, userDetails.UserId);
                    ...
                    iEarningsSum = SetEarningsDeductionsRange(iRowIndex, userDetails.UserId, iEarningsSum, ref iDeductionSum, dcTotalDays, iDaysOfMonth, aiMonthId, userDetails.Gender, userDetails.StaffGroupId);
                    ...
                    {
                        string sDA = moDTTempSalaryDetails.Rows[iRowIndex]["D.A."].ToString();
                        int iDAAmount = sDA.Substring(0, sDA.IndexOf("_")).ToInt();
                        decimal iPF13 = Math.Round((decimal)(iDAAmount * 13) / 100, 0);

                        moDTTempSalaryDetails.Rows[iRowIndex]["PF 13"] = iPF13;

                        string sHRA = moDTTempSalaryDetails.Rows[iRowIndex]["H.R.A."].ToString();
                        int iHRAAmount = sHRA.Substring(0, sHRA.IndexOf("_")).ToInt();

                        string sAttendanceAllawnce = moDTTempSalaryDetails.Rows[iRowIndex]["A.A."].ToString();
                        int iAttendanceAllowanceAmount = sAttendanceAllawnce.Substring(0, sAttendanceAllawnce.IndexOf("_")).ToInt();
                        decimal iESIC3_25 = Math.Round(((iDAAmount + iHRAAmount + iAttendanceAllowanceAmount) * 3.25) / 100, 0).ToDecimal();

                        string sESIC75 = moDTTempSalaryDetails.Rows[iRowIndex]["ESIC"].ToString();
                        int iESIC75Amount = sESIC75.Substring(0, sESIC75.IndexOf("_")).ToInt();

                        if (iESIC75Amount == 0)
                            iESIC3_25 = 0;

                        moDTTempSalaryDetails.Rows[iRowIndex]["ESIC 3.25"] = iESIC3_25;

                        string sMLWP = moDTTempSalaryDetails.Rows[iRowIndex]["MLWP 25"].ToString();
                        int iMLWPAmount = sMLWP.Substring(0, sMLWP.IndexOf("_")).ToInt() * 3;
                        moDTTempSalaryDetails.Rows[iRowIndex]["MLWP 75"] = iMLWPAmount;

                        string sGrossSalary = moDTTempSalaryDetails.Rows[iRowIndex]["Gross Salary"].ToString();
                        moDTTempSalaryDetails.Rows[iRowIndex]["CTC"] = sGrossSalary.ToInt() + iPF13 + iESIC3_25 + iMLWPAmount;
                    }
        ...
        private void DesignFormula(int iDaysOfMonth, decimal dcTotalDays, ref int iEDFormulaValue, List<UsersEarnDeductDetails> UsersEDForFormula, Dictionary<int, decimal> dictEDValue, ref string sFormula, ref string sTempFormula, ref decimal iTempEDValue, int aiUserId)
        {
            foreach (var UsersED in UsersEDForFormula)
            {
                ...
                UserSpecialCaseDetails oUserSpecialCaseDetails = SalaryEntityLists.lstUserSpecialCaseDetails.Where(uscd => uscd.UserId == aiUserId && uscd.EarningDeductionName.ToUpper() == UsersED.ShortName.ToUpper()).FirstOrDefault();
                if (oUserSpecialCaseDetails != null && oUserSpecialCaseDetails.MultiplierValue != 0)
                    iEDFormulaValue = Math.Round(iEDFormulaValue * oUserSpecialCaseDetails.MultiplierValue, 0).ToInt();
        ...
        private void SetUsersEarningsDeductions(int iDaysOfMonth, ref int iEarningsSum, ref int iDeductionSum, int iRowIndex, decimal dcTotalDays, List<UsersEarnDeductDetails> UsersEarningsDeductions, int aiUserId)
        {
            int EDValue = 0;
            ...
            foreach (UsersEarnDeductDetails UsersED in UsersEarningsDeductions)
            {
                ...
                if (Convert.ToBoolean(UsersED.IsAttendanceDependent) == true)
                {
                    UserSpecialCaseDetails oUserSpecialCaseDetails = SalaryEntityLists.lstUserSpecialCaseDetails.Where(uscd => uscd.UserId == aiUserId && uscd.EarningDeductionName.ToUpper() == UsersED.ShortName.ToUpper()).FirstOrDefault();
                    if (oUserSpecialCaseDetails != null && oUserSpecialCaseDetails.MultiplierValue != 0)
                        EDValue = Math.Round(EDValue * oUserSpecialCaseDetails.MultiplierValue, 0).ToInt();
        ...
        private int SetEarningsDeductionsRange(int iRowIndex, int aiUserId, int iEarningsSum, ref int iDeductionSum, decimal dcTotalDays, int iDaysOfMonth, int aiMonthId, char acGender, int aiStaffGroupId)
        {
            ...
            foreach (var EDRange in EarningsDeductionsRange)
            {
                ...
                if (ApplyStandardPTFormula)
                {
                    if (iAmount == 175 && acGender == 'F')
                        iAmount = 0;

                    if (iEarningsSum <= 25000 && acGender == 'F')
                        iAmount = 0;
                }
                else
                {
                    if(acGender == 'F')
                    {
                        if(aiStaffGroupId == 31) // General
                        {
                            if (iEarningsSum < 25000)
                                iAmount = 0;
                            else if (iAmount == 300)
                                iAmount = 200;
                        }
                        ...
                        {
                            if (iEarningsSum <= 9999)
                                iAmount = 0;
                            if (iEarningsSum == 10000)
                                iAmount = 175;
                            if (iEarningsSum > 10000)
                                iAmount = 200;
                        }
        ...
        private decimal SetAttendanceDetails(int iRowIndex, decimal dcUnpaidLeaves, int aiUserId, int aiDaysOfMonth)
        {
            ...
            //if (ExcludeWeekends)
            //{
            //    dcTotalDays = dcTotalDays - miSundaysCount;
            //    dcAttendance = dcAttendance - miSundaysCount;
            //}
        ...
        private void AddEarningDeductionsColumns()
        {
            ...
            }
            {
                ...
                if (ShowAdditionalColumns)
                {
                    moDTSalaryDetails.AddColumns(new string[] { "PF 13", "ESIC 3.25", "MLWP 75", "CTC" });
                    moDTTempSalaryDetails.AddColumns(new string[] { "PF 13", "ESIC 3.25", "MLWP 75", "CTC" });
                }
        ...
        public List<UsersEarningsDeduction> GetSalaryStructureOfUser(int aiUserId)
        {
            ...
            foreach (var EDFormula in EarningsDeductionsFormulae)
            {
                ...
                DesignFormula(iDaysOfMonth, iDaysOfMonth, ref iEDFormulaValue, UsersEDForFormula, dictEDValue, ref sFormula, ref sTempFormula, ref iTempEDValue, aiUserId);
        ...
        {
            return moSalaryDetailsDC.GetUsers(aiStaffGroupId);
        }

        private int GetWeekdayCount(int aiYear, int aiMonth, DayOfWeek aoDayOfWeek)
        {
            if (ExcludeWeekends)
            {
                int iCount = Enumerable.Range(1, DateTime.DaysInMonth(aiYear, aiMonth))
                .Select(day => new DateTime(aiYear, aiMonth, day))
                .Count(date => date.DayOfWeek == aoDayOfWeek);
