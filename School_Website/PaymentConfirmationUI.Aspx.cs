...
public partial class PaymentConfirmationUI : SchoolBase
{
    ...
    {
        if (miGatewayId == Constants.PaymentGateways.Billdesk.ToInt())
        {
	...
	protected void btnConfirm_Click(object sender, EventArgs e)
	{
		try
		{
			...
			else
			{
                ...
                List<PaymentGateWayDetails> lstPaymentGateWayDetails = NetBankingPaymentTransactionsBL.GetPaymentGatewayDetails(sFeeIds);
                if (miGatewayId == Constants.PaymentGateways.TPSL.ToInt())
                {
                ...
                else if (miGatewayId == Constants.PaymentGateways.PayU.ToInt())
                {
                    ...
                    if (miSchoolId == Utility.Constants.SchoolId.PPS.ToInt())
                        bUseAlternate = true;
    ...
    private string GetStudentFeeIds()
    {
        ...
        if(Session[S_SCHOOWISE_STUDENT_FEE_ID] != null)
            oStudentFeeIds = Session[S_SCHOOWISE_STUDENT_FEE_ID] as List<int>;
    ...
    {
        NetBankingPaymentTransactionsBL oNetBankingPaymentTransactionsBL = new NetBankingPaymentTransactionsBL();
        List<OnlinePaymentType> lstOnlinePaymentType = oNetBankingPaymentTransactionsBL.GetOnlinePaymentTypes();
    ...
    private void SetPaymentTypeState()
    {
        ...
        if (miGatewayId == Convert.ToInt32(Constants.PaymentGateways.Atom) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.AxisBankForAll) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.Billdesk) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.EaseBuzz) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.PayUMoney) || (miGatewayId == Convert.ToInt32(Constants.PaymentGateways.PayU) && (miSchoolId == Constants.SchoolId.SNS.ToInt() || miSchoolId == Constants.SchoolId.DSK.ToInt())))
        {
    ...
    private int CreateNextYearFeeTransaction(NetBankingTransaction aoNetBankingTransaction, string asRemarks)
    {
        ...
        DataTable oDataTable = oNetBankingPaymentTransactionsBL.PayStudentNextYearFeeOnLine(iStandardId, asRemarks, sDueDatesFilterXML, iLateFeeAmount, aoNetBankingTransaction, bIsForInternalFee, sInternalFeeDetailsIds);
    ...
    private int CreateCurrentYearFeeTransaction(NetBankingTransaction aoNetBankingTransaction, int aiStudentId, string asRemarks)
    {
        ...
        DataTable oDataTable = oNetBankingPaymentTransactionsBL.PayStudentFeeOnLine(iLateFeeAmount, asRemarks, sLateFeeRemark, sStudentFeeIdXML, aoNetBankingTransaction, bIsPayFromMobile, bIsCautionMoneyPayment, bIsInternalFeePayment);
        return oDataTable.Rows[0][0].ToInt();
    }
    ...
    private int CreateAdmissionTransaction(NetBankingTransaction aoNetBankingTransaction)
    {
        ...
        if (iAdmissionId != 0)
            iTransactionID = oNetBankingPaymentTransactionsBL.CreateNetBankingTransaction(aoNetBankingTransaction, iAdmissionId);
        return iTransactionID;
    }
	...
	private string GetStudentFeeIdXML()
	{
		...
		foreach (int iStudentFeeId in oStudentFeeIds)
		{
