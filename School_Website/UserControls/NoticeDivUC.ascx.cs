...
public partial class NoticeDivUC : UserControl
{
    ...
    protected void ObjDSNotices_selecting(object sender, ObjectDataSourceSelectingEventArgs e)
    {
        try
        {
			...
			e.InputParameters["aiSchoolId"] = ConfigurationManager.AppSettings["SchoolID"];
        }
        catch (Exception ex)
        {
    ...
    protected void lstvwNotices_ItemDataBound(object sender, ListViewItemEventArgs e)
    {
        try
        {
            ...
            if (lstvwNotices.DataKeys[oCurrentItem.DisplayIndex]["FileName"].ToString() != "" && lstvwNotices.DataKeys[oCurrentItem.DisplayIndex]["FileName"] != null)
            {   
                oHyperLink.Attributes.Add("onclick", "window.open('" + msServerPath + lstvwNotices.DataKeys[oCurrentItem.DisplayIndex]["FileName"].ToString()
                                   + "' , '_blank','scrollbars=yes,resizable=no,top=0,left=0,width=800,height=600'); return false;");


                oHyperLink.NavigateUrl = "~/RITeSchool/DOWNLOADS/School Notices/" + lstvwNotices.DataKeys[oCurrentItem.DisplayIndex]["FileName"].ToString();
            }
            else
            {
    ...
    {
        if (Session[Constants.S_SESSION_ENABLE_LOGIN_FOR_LEFT_STUDENTS] != null && Session[Constants.S_SESSION_ENABLE_LOGIN_FOR_LEFT_STUDENTS].ToString() == "1")
        {
            divSchoolNoticesLink.Visible = false;
        }
        else
        {
            ...
            List<NoticeDetails> lstNotice = oNoticeDetailsBL.GetNotices(Convert.ToInt32(ConfigurationManager.AppSettings["SchoolID"]), DisplayLocation, false, string.Empty, string.Empty, Constants.I_GRID_PAGE_COUNT, Constants.I_ZERO, moUserRole.ToInt());
            List<NoticeDetails> lstNoticeDetails = new List<NoticeDetails>();

            if (Session[Constants.S_SESSION_USER_LOGIN_ROLE_ID] != null && (((Constants.UserRoles)Session[Constants.S_SESSION_USER_LOGIN_ROLE_ID]) == Constants.UserRoles.Student))
            {
                int StandardDivisionIdNotice = oNoticeDetailsBL.GetStandarDivisionId(Convert.ToInt32(ConfigurationManager.AppSettings["SchoolID"]), Convert.ToInt32(Session[Constants.S_SESSION_USER_ID]));
                lstNotice.ForEach(nd =>
                {
                    var sIds = nd.ClassesIds.Split(',');
                    foreach (string id in sIds)
                    {
                        if (id.Trim() == StandardDivisionIdNotice.ToString())
                        {
                            lstNoticeDetails.Add(nd);
                            break;
                        }
                    }
                });
            ...
            {
                lstNoticeDetails = lstNotice;
            }

            lstvwNotices.DataSource = lstNoticeDetails;
