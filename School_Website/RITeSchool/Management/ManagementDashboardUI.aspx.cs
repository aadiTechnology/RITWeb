...
{
	public partial class ManagementDashboardUI : SchoolBase
	{
		#region -- PROPERTIES --
		...
		/// </summary>
		private MISServiceClientUtility ManagementServiceClient
		{
			get
			{
				var mgmtServiceClient = Session[Constants.S_SESSION_MANAGEMENT_CLIENT] as MISServiceClientUtility;
				if (mgmtServiceClient == null)
				{
					mgmtServiceClient = new MISServiceClientUtility();
		...
		protected void studentAttendanceDate_TextChanged(object sender, EventArgs e)
		{
			try
			{
				...
				ManagementServiceClient.GetStudentMISDetails();
				SerializeMISDataInJSONFormat();
			}
			catch (Exception ex)
			{
		...
		/// </summary>
		private void SerializeMISDataInJSONFormat()
		{
			hidJSON.Value = ManagementServiceClient.GetJSONSerializedData();
            HidUserId.Value = Session[Constants.S_SESSION_SUPER_ADMIN_USER_ID].ToString();
		}
		...
		/// <summary>
		/// This method is used to set query string to open change password popup
		/// </summary>
		private void SetChangePasswordLink()
		{
			if (QueryString["SuperAdminDetailsId"] != null)
			{
				string sQueryString = "IsFromManagementDashboard=Y";
				string sEncryptQueryString = Utility.CommonUtility.EncryptQuerystring(sQueryString);
				lnkChangePwd.Attributes.Add("onclick", "window.open('../Common/StudentChangePassword.aspx?" +
			sEncryptQueryString + "', '_new','scrollbars=yes,resizable=no,top=0,left=0,width=600,height=370px');return false;");
			}
		}

        [WebMethod]
        public static string GetQueryString(string asId, string aiUserId)
        {
            DataTable oDataTable = ManagementServiceConfigBL.GetManagementUserInfo(asId.ToInt(), aiUserId.ToInt());
            string sPassword = CommonUtility.GetDecryptedPassword(oDataTable.Rows[0]["User_Login"].ToString(), oDataTable.Rows[0]["User_Password"].ToString());
            string sQueryString = oDataTable.Rows[0]["LoginPageURL"].ToString() + "?" + CommonUtility.EncryptQuerystring("IsManagementUser=1&UserName=" + oDataTable.Rows[0]["User_Login"].ToString() + "&Password=" + sPassword);
            return sQueryString;
        }
