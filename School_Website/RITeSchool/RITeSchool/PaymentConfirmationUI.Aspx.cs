...
public partial class PaymentConfirmationUI : SchoolBase
{
    ...
    private const string S_PAYABLE_AMOUNT_MESSAGE = "%INCLUDE% processing charge & service tax.";
		...
		try
		{
			...
			if (!IsPostBack)
			{
                ...
                if (moUserRole == Constants.UserRoles.Student && hidTransactionFrom.Value != S_ADMISSION)
                    ...
                    SubmissionWizardSteps.ActiveSteps = 3;              
			}
            if (moUserRole == Constants.UserRoles.Student && hidTransactionFrom.Value != S_ADMISSION)
    ...
    private void SetFieldState()
    {
        ...
        if(iSchoolId == Constants.SchoolId.BFS.ToInt())
            trServiceTax.Visible = false;
    }
	...
	protected void cmbBanks_SelectedIndexChanged(object sender, EventArgs e)
	{
		try
		{
            ...
            //{
            //    RazorPayPGRequest oRazorPayPGRequest = new RazorPayPGRequest();
            //    oRazorPayPGRequest.CalculateFeeAmount(dtRow, out dDeductedAmount, out dFormFees, dServiceTaxAmount, dAmount, this);
            //}
	...
	protected void btnConfirm_Click(object sender, EventArgs e)
	{
		try
		{
			...
			else
			{
                ...
                List<PaymentGateWayDetails> lstPaymentGateWayDetails = NetBankingPaymentTransactionsBL.GetPaymentGatewayDetails(sFeeIds, iScStudentId, hidSelectedFeeTypes.Value, iNxtAcYearId);
                if (miGatewayId == Constants.PaymentGateways.TPSL.ToInt())
                {
                ...
                else if (miGatewayId == Constants.PaymentGateways.CCAvenueVPMCPS.ToInt())
                {
                    ...
                    string sPaymentFor = (Session[Constants.S_TRANSACTION_FROM].ToString() == S_ADMISSION ? "Admission" : "StudentFee");
                    CCAvenueVPMCPSPGRequest oCCAvenueVPMCPSPGRequest = new CCAvenueVPMCPSPGRequest(sReturnURL, oStudentNetBankingDetails.FirstName, oStudentNetBankingDetails.Email, oStudentNetBankingDetails.Phone);
                ...
                //    string sPaymentFor = (Session[Constants.S_TRANSACTION_FROM].ToString() == S_ADMISSION ? "Admission" : "StudentFee");
                //    RazorPayPGRequest oRazorPayPGRequest = new RazorPayPGRequest(sReturnURL, oStudentNetBankingDetails.FirstName, oStudentNetBankingDetails.Email, oStudentNetBankingDetails.Phone);
                ...
                //    string sQueryString = "Amount=" + sPayAmt + "&TxnId=" + iTransactionID + "&SchoolId=" + miSchoolId +
                //                            "&Name=" + oStudentNetBankingDetails.FirstName + "&Email=" + oStudentNetBankingDetails.Email + "&ContactNo=" + oStudentNetBankingDetails.Phone;
                //    Response.Redirect("http://localhost:2799/RequestPayment.aspx?"+CommonUtility.EncryptQuerystring(sQueryString),false);
                //    //hidParameters.Value = oRazorPayPGRequest.SendRequest(sPayAmt, iTransactionID.ToString(), oPaymentGateWayDetails, sPaymentFor, sGuid);
                //}
    ...
    private void FillPaymentTypes()
    {
        ...
        //{
        //    lstOnlinePaymentType.Add(new OnlinePaymentType { Id = 1, Type = "Axis Gateway" });
        //    lstOnlinePaymentType.Add(new OnlinePaymentType { Id = 2, Type = "RazorPay Gateway" });
        //}
        //else
    ...
    {   
        ...
        if (miGatewayId == Convert.ToInt32(Constants.PaymentGateways.Atom) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.RazorPay) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.CCAvenue) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.CCAvenueVPMCPS) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.AxisBankForAll) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.Billdesk) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.BilldeskDYP) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.EaseBuzz) || miGatewayId == Convert.ToInt32(Constants.PaymentGateways.PayUMoney) || (miGatewayId == Convert.ToInt32(Constants.PaymentGateways.PayU) && (miSchoolId == Constants.SchoolId.SNS.ToInt() || miSchoolId == Constants.SchoolId.DSK.ToInt())))
        {
            ...
            //    Span1.InnerText = "Select Gateway";
            //}
            //else
...
public class CCAvenueVPMCPSPGRequest : Page
{
    ...
    public CCAvenueVPMCPSPGRequest(string asReturnURL, string asFirstName, string asEmail, string asPhone)
    {
        this.msFirstName = asFirstName;
        this.msEmail = asEmail;
        this.msPhone = asPhone;
...
//public class RazorPayPGRequest : Page
//{
//    #region "Data Member(s)"
...
//    public RazorPayPGRequest()
//    {
//    }

//    public RazorPayPGRequest(string asReturnURL, string asFirstName, string asEmail, string asPhone)
//    {
//        this.msFirstName = asFirstName;
//        this.msEmail = asEmail;
//        this.msPhone = asPhone;
...
//        string sCurrencyType = "INR";

//        int iAmount = asAmount.ToDecimal().ToInt() * 100;

//        Dictionary<string, object> input = new Dictionary<string, object>();
//        input.Add("amount", iAmount);
//        input.Add("currency", sCurrencyType);
//        input.Add("receipt", asTransactionId);
//        input.Add("payment_capture", 1);

//        string key = aoPaymentGateWayDetails.AccessCode;
//        string secret = aoPaymentGateWayDetails.Hash;

//        System.Net.ServicePointManager.SecurityProtocol = (SecurityProtocolType)3072;
//        RazorpayClient client = new RazorpayClient(key, secret);

//        Razorpay.Api.Order order = client.Order.Create(input);
//        string sOrderId = order["id"].ToString();

//        StringBuilder sb = new StringBuilder();
//        sb.Append("src=" + aoPaymentGateWayDetails.NetBankingUrl);
//        sb.Append(",data-key=" + aoPaymentGateWayDetails.AccessCode);

//        sb.Append(",data-amount=" + iAmount);
//        sb.Append(",data-name=" + aoPaymentGateWayDetails.ProductInfo);
//        sb.Append(",data-description=" + string.Empty);
//        sb.Append(",data-order_i=" + sOrderId);
//        sb.Append(",data-prefill.name=" + msFirstName);

//        sb.Append(",data-prefill.email=" + msEmail);
//        sb.Append(",data-prefill.contact=" + msPhone);
//        sb.Append(",data-theme.color=" + string.Empty);

//        //sb.Append(",data-notes.Guid=" + asGuid);
