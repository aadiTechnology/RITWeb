...
public partial class PaymentStatusUI : SchoolBase
{
    ...
    private void DisplayErrorMessage()
    {
        ...
        {
            string sEmailAddress = ConfigurationManager.AppSettings["FromMailAddress"].ToString();
	...
	private string SendSMS(int aiAdmissionId)
	{
		int iSchoolId = ConfigurationManager.AppSettings["SchoolID"].ToInt();		
		...
		if (oDataTable.Rows.Count > 0)
		{
            ...
            return String.Format("iAdmissionId={0}&Form_Number={1}&Mobile_Number={2}&EnableAdmissionFormFee=true", aiAdmissionId, sFormNumber, sMobileNumber);
		}
		return string.Empty;
	}
	...
	private void ProcessNetBankingTransaction()
    {
        try
        {
            ...
            if (!sTransactionFrom.IsNullOrEmpty())
            {
                ...
                else if (sGateway == Constants.PaymentGateways.Atom.ToString() || sGateway == Constants.PaymentGateways.PayUMoney.ToString() || sGateway == Constants.PaymentGateways.AxisBankForAll.ToString() || sGateway == Constants.PaymentGateways.EaseBuzz.ToString() ||
                    sGateway == Constants.PaymentGateways.Billdesk.ToString())
                {
    ...
    private void CompleteOnlineAdmission(NetBankingTransaction oNetBankingTransaction)
    {
        ...
        string sParams = SendSMS(oDataTable.Rows[0]["AdmissionId"].ToInt());
    ...
    private void CompleteStudentFeePayment(NetBankingTransaction oNetBankingTransaction, Constants.OnlineFeeTypes aoOnlineFeeTypes)
    {
        ...
        string sQryString = CommonUtility.EncryptQuerystring("TransactionStatus=" + true.ToString());
...
public class PayUPGResponseData : SchoolBase
{
    ...
    public NetBankingTransaction ReadNetBankingData(out bool abIsChecksumMatched)
    {
        PaymentGateWayDetails oPaymentGateWayDetails = NetBankingPaymentTransactionsBL.GetPaymentGatewayDetails(GetStudentFeeIds()).Where(a => a.GatewayId == Constants.PaymentGateways.PayU.ToInt()).FirstOrDefault();
        NetBankingTransaction oNetBankingTransaction = new NetBankingTransaction();
        abIsChecksumMatched = false;
...
public class PayUMoneyPGResponse : System.Web.UI.Page
{
    ...
    {
        PaymentGateWayDetails oPaymentGateWayDetails = NetBankingPaymentTransactionsBL.GetPaymentGatewayDetails(GetStudentFeeIds()).Where(a => a.GatewayId == Constants.PaymentGateways.PayUMoney.ToInt()).FirstOrDefault();
        NetBankingTransaction oNetBankingTransaction = new NetBankingTransaction();
...
public class AxisBankForAllPGResponse : System.Web.UI.Page
{
    ...
    public NetBankingTransaction ReadNetBankingData(NameValueCollection asResponse, string asReceivedHash)
    {
        ...
        if (asResponse.Count > 0)
        {
            ...
            {
                if (asResponse["PMD"].ToString() == "CD" || asResponse["PMD"].ToString() == "OIB")
                {
...
public class EasebuzzPGResponse : System.Web.UI.Page
{
    ...
    {
        PaymentGateWayDetails oPaymentGateWayDetails = NetBankingPaymentTransactionsBL.GetPaymentGatewayDetails(GetStudentFeeIds()).Where(a => a.GatewayId == Constants.PaymentGateways.EaseBuzz.ToInt()).FirstOrDefault();
...
public class BillDeskPGResponse : System.Web.UI.Page
{
    ...
    public NetBankingTransaction ReadNetBankingData(string asQueryString)
    {
        ...
        if (oResponse.Count > 0)
        {
            ...
            {
                PaymentGateWayDetails oPaymentGateWayDetails = NetBankingPaymentTransactionsBL.GetPaymentGatewayDetails(GetStudentFeeIds()).Where(a => a.GatewayId == Constants.PaymentGateways.Billdesk.ToInt()).FirstOrDefault();
