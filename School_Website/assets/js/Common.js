...
function collapseSidebarAsPerScreenWidth() {
        $("label, body, textarea").css("font-size", "11px");
        $("#sidebar").addClass("sidebar menu-min");
}

function setTextAreaWidthAccordingToScreen() {
    if ($(window).width() < 1200) {
        $(".k-edit-field textarea").css("width", "140px");
    }
    else {
        $(".k-edit-field textarea").css("width", "155px");
    }
}
...
function applyReadOnlyEffectToEditPopup() {
    ...
    $(".k-numeric-wrap.k-state-default").replaceWith(function () {
        return this.childNodes[0].childNodes[0];
    });
...
function showRequiredFields(textBoxControls, dropDownControls, textareaControls, numberControls) {
$(".k-edit-form-container input[type=text]").each(function () {
   ...
   $(".k-numeric-wrap.k-state-default").each(function () {
       if (numberControls.split(",").indexOf($(this).closest(".k-edit-field").attr("data-container-for")) > -1) {
           $(this).closest(".k-edit-field").append('<span class="required">*</span>');
       }
   });  
}
...
function showCommaSeparatedAmount(amount, thousandSeparator, decimalSeparator) {    
    if (amount)
        return amount.toString().replace('.', decimalSeparator).replace(/\B(?=(\d{3})+(?!\d))/g, thousandSeparator);
    else
        return '';
}
...
function openPopUp(url, windowName, width, height) {
    ...
    while (parentWindow != undefined && parentWindow != null) {
        if (parentWindow.opener != undefined && parentWindow.opener != null)
            parentWindow = parentWindow.opener;
        else
            break;
    }
    var left = ($(parentWindow).innerWidth() - width) / 2;
    if ($(parentWindow).innerWidth() < width) {
        width = $(parentWindow).innerWidth() * 0.90;
    }

    var top = ($(parentWindow).innerHeight() - height) / 2;
    if ($(parentWindow).innerHeight() < height) {
        height = $(parentWindow).innerHeight() * 0.90;
    }
    $(".k-upload-status.k-upload-status-total").hide();
    var newPopupWindow = window.open(url, windowName.replace(/ /g, ''), 'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=no, width=' + width + ', height=' + height + ', top=' + top + ', left=' + left);
    newPopupWindow.focus();
    return newPopupWindow;
}

// This function is used to remove the special character * & ' from the string.
function restrictSpecialCharacter(event) {
